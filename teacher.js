...
(function () {
  ...
  function renderTeacher() {
    ...
    html += '<div class="hero"><h1>교사용 전체 미리보기<small>TEACHER CONSOLE · ANSWER KEY · QR</small></h1><p>모든 증거의 분석 방법, 감식 결과, 추론 질문과 카드 뒷면용 QR을 한눈에 확인합니다. 이 페이지 주소는 학생에게 공개하지 않습니다.</p></div>';
    ...
    D.cases.forEach(function (c) {
      ...
      c.evidence.forEach(function (ev) {
        ...
        html += '<dl><dt>의뢰 항목 (카드 뒷면)</dt><dd>' + esc(ev.request) + '</dd><dt>분석 방법</dt><dd>' + esc(ev.methodName) + ' — ' + esc(ev.methodNote) + '</dd><dt>핵심 감식 결과 (학생이 옮겨 쓰는 문장)</dt><dd class="ans">' + esc(ev.key) + '</dd><dt>함께 확인된 것</dt><dd><ul>' + (ev.more || []).map(function (r) { return '<li>' + esc(r) + '</li>'; }).join('') + '</ul></dd><dt>추론 연결고리</dt><dd>' + esc(ev.link || '') + '</dd><dt>추론 질문</dt><dd><ul>' + ev.inference.map(function (r) { return '<li>' + esc(r) + '</li>'; }).join('') + '</ul></dd></dl></div></div>';
      });
